...
if [ $# != 9 ]; then
	echo "Usage: $scriptname <R1 FASTQ file> <S/I quality> <Y/N uniq> <length_cutoff; 0 for no length_cutoff> <Y/N keep short reads> <adapter_set> <start_nt> <crop_length> <quality_cutoff>"
	exit
fi
...
then
	...
	START1=$(date +%s)
	cutadapt_quality.csh $basef.modheader.fastq $quality $length_cutoff $keep_short_reads $adapter_set $quality_cutoff
	mv $basef.modheader.cutadapt.fastq $basef.cutadapt.fastq
	rm -f $basef.modheader.fastq
else
	START1=$(date +%s)
	cutadapt_quality.csh $inputfile $quality $length_cutoff $keep_short_reads $adapter_set $quality_cutoff
fi

...
if [ $# != 11 ]; then
	echo "Usage: $scriptname <R1 FASTQ file> <S/I quality> <Y/N uniq> <length cutoff; 0 for no length cutoff> <# of cores> <Y/N keep short reads> <adapter_set> <start_nt> <crop_length> <quality_cutoff> <prefix>"
	exit
fi
...
split -l "$SplitPerCore" "$inputfile" "$prefix"
...
do
	...
	preprocess.sh "$f.fastq" "$quality" N "$length_cutoff" "$keep_short_reads" "$adapter_set" "$start_nt" "$crop_length" "$quality_cutoff" >& "$f.preprocess.log" &
done
